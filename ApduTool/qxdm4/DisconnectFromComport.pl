...
sub ParseArguments
{
   # Assume failure
   my $RC = false;
   my $Help = "Syntax: Perl DisconnectFromComport.pl [Optional COM Port Number]\n"
            . "Eg:     Perl DisconnectFromComport.pl 5\n";
...
sub DumpCOMPort
{
   ...
   {
      print "Failed to disconnect. Connected to port: COM" . $COMPort . "\n";
   }
}
...
sub Execute
{
  ...
  if ($PortNumber != -1)
   {
      # Change the COM port
      $QXDM->DisconnectFromComport($PortNumber);
   }
