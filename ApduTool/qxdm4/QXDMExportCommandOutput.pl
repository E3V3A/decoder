...
# Generate item store file name
   ...
   $RC = $QXDM->ExportViewText( "Command Output", $FileName );
   if ($RC == false)
   {
...
sub Execute
{
 ...
 # Export all items from item view
   ...
   if ($RC == false)
   {
      print "\nUnable to create view, 'Command Output' not found\n";

      return;
   }
   
	$QXDM->SendScript("nv_write_item 881 1");
	$QXDM->SendScript("nv_write_item 882 0");
	$QXDM->SendScript("nv_write_item 850 0 2");
